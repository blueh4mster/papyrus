...
export async function lockFunds({
  ...
  // const tx_weth = await weth.deposit({ value: ethers.utils.parseEther("0.1") });
  // await tx_weth.wait();
  ...
  // await weth.approve(HTLC_ADDRESS, ethers.utils.parseEther("0.1"));
...
export async function watchHTLCEvents(callback: Function) {
  ...
  contract.on("Locked", (id, sender, receiver, amount, hashlock, timelock, event) => {
    callback({
      id,
      sender,
      receiver,
